...
| `scripts/trigger_batch.ps1` | **정시 트리거**(Windows 작업 스케줄러 `nps-tracker-daily-batch`, 평일 15:45 KST) — GitHub cron은 실측 2~3시간 지연되므로 정시성은 로컬 트리거가 담당, GitHub cron은 PC 꺼짐 대비 백업. 재등록: `Register-ScheduledTask`(스크립트 상단 주석 참조) |
